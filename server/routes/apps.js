...
router.post('/', authenticateToken, asyncHandler(async (req, res) => {
  ...
  if (templateId) {
    ...
    if (template && template.app_schema) {
      try {
        ...
        console.log('Note: AppSchema creation skipped - model structure needs enhancement for template schemas');
      }
    }
  ...
  res.status(201).json(createSuccessResponse('App created successfully', {
    app: sanitizeApp(app)
  }));
}));
...
router.get('/:id', authenticateToken, asyncHandler(async (req, res) => {
  ...
  res.json(createSuccessResponse('App retrieved successfully', {
    app: appData
  }));
}));
...
router.get('/:id/status', authenticateToken, asyncHandler(async (req, res) => {
  ...
  const generateIndustrySpecificMetrics = (app) => {
    ...
    const baseMetrics = {
      ...
      trafficStats: {
        ...
        peakHour: isActive ? `${Math.floor(Math.random() * 12) + 9}:00` : null,
    ...
    if (isActive && activeDays > 0) {
      ...
      for (let i = 0; i < issueCount; i++) {
        issuesAndWarnings.push({
          ...
          timestamp: new Date(Date.now() - Math.random() * activeDays * 24 * 60 * 60 * 1000).toISOString(),
    ...
    return {
      ...baseMetrics,
      downtimeLogs: downtimeLogs.sort((a, b) => new Date(b.date) - new Date(a.date)),
      issuesAndWarnings: issuesAndWarnings.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp))
    };
  };
  ...
  res.json(createSuccessResponse('App status retrieved successfully', {
    app: sanitizeApp(app),
    metrics: mockMetrics
  }));
}));
...
router.put('/:id/status', validate(schemas.idParams, 'params'), validate(schemas.updateAppStatus), async (req, res) => {
  try {
